...
document.addEventListener("DOMContentLoaded", async function () {
	...
	if (closeEditBookBtn) {
		...
		console.warn(
			"Botão '.close-edit-book' para fechar o modal não encontrado.",
		);
	}
	...
	if (editBtnInInfModal) {
		editBtnInInfModal.addEventListener("click", async () => {
			...
			if (!bookIdToEdit) {
				console.error(
					"Erro: ID do livro para editar não encontrado no dataset do botão.",
				);
			...
			if (typeof window.prepareBookModalForEdit === "function") {
				...
				console.error(
					"Função 'window.prepareBookModalForEdit' não encontrada. Verifique o escopo em create_book.js.",
				);
				alert(
					"Erro: A função de edição não está disponível. Contate o suporte.",
				);
			}
		});
	}
	...
	if (deleteBtnInInfModal) {
		deleteBtnInInfModal.addEventListener("click", async () => {
			...
			if (!bookIdToDelete) {
				console.error(
					"Erro: ID do livro para deletar não encontrado no dataset do botão.",
				);
				alert(
					"Erro: ID do livro não disponível para deleção. Tente novamente.",
				);
				return;
			}
			...
			if (
				...
				if (typeof window.deleteBook === "function") {
					...
					alert(
						"Erro: A função de deleção não está disponível. Contate o suporte.",
					);
				}
			}
		});
